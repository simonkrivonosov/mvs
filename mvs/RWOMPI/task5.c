...
void stats(void *datat, int *distribution, struct timeval start) {
  data_t *ctx = datat;
  FILE *fd;
  int check = 0;
...
int getdestination(void *datat, int direction) {
  data_t *ctx = datat;
...
void finish(void *datat, int *completedsize, int *done,
